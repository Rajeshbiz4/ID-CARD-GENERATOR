...
const schoolSchema=new Schema({
  ...
  name:{type:String,required:true,trim:true},contactPersonName:String,email:String,mobile:String,
  registrationNo:String,address:String,city:String,state:String,pinCode:String,website:String,tagline:String,
  logoFileId:{type:Schema.Types.ObjectId,default:null},principalName:String,
  principalSignatureFileId:{type:Schema.Types.ObjectId,default:null},
...
const studentSchema=new Schema({
  ...
  admissionNo:{type:String,required:true,trim:true},name:{type:String,required:true,trim:true},
  rollNo:{type:String,required:true,trim:true},className:{type:String,required:true,trim:true},
  division:{type:String,required:true,trim:true},gender:{type:String,enum:["Male","Female","Other"],default:"Other"},
  dob:Date,bloodGroup:String,academicYear:String,parentName:String,parentMobile:String,emergencyContact:String,
  address:String,house:String,busRoute:String,photoFileId:{type:Schema.Types.ObjectId,default:null},
  cardStatus:{type:String,enum:["PENDING","GENERATED"],default:"PENDING"},isDeleted:{type:Boolean,default:false}
},{timestamps:true});
studentSchema.index({schoolId:1,admissionNo:1},{unique:true});

const decorationSchema=new Schema({
  type:{type:String,enum:["rect","circle","line"],required:true},
  x:Number,y:Number,width:Number,height:Number,color:String,opacity:{type:Number,default:1},rotate:{type:Number,default:0}
},{_id:false});
...
const templateSchema=new Schema({
  ...
  category:{type:String,default:"Modern"},layoutFamily:{type:String,default:"Centered"},
  orientation:{type:String,enum:["portrait","landscape"],default:"portrait"},
  width:{type:Number,default:330},height:{type:Number,default:520},backgroundColor:{type:String,default:"#ffffff"},
  backgroundFileId:{type:Schema.Types.ObjectId,default:null},
  decorations:{type:[decorationSchema],default:[]},elements:{type:[elementSchema],default:[]},
  active:{type:Boolean,default:true},isDeleted:{type:Boolean,default:false}
},{timestamps:true});
...
export const TemplateSettings=model("TemplateSettings",templateSettingsSchema);
